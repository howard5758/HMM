...
class Maze:
    ...
    def state_num(self, x, y):
        state = -1
        for i in range(0, self.index(x, y) + 1):
            if not self.map[i] == "#":
                state = state + 1
    ...
    def find_neighbors(self, x, y):
        ...
        neighbors = [(x, y)]
        for i in range(0, len(dirr)):
            if self.is_floor(dirr[i][0], dirr[i][1]):
                neighbors.append((dirr[i][0], dirr[i][1]))
        return neighbors
...
if __name__ == "__main__":
    ...
    print(test_maze1)
    print(test_maze1.num_state())
    print(test_maze1.state_num(2, 2))
    print(test_maze1.find_neighbors(1, 1))
